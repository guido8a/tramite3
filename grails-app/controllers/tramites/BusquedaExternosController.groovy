...
class BusquedaExternosController {
    ...
    def tablaBusquedaExternos() {
        ...
        if (filtrados.size() >= 1) {
            ...
            msg += "<h4>Trámite: <strong class='text-success'> ${externo.codigo} </strong></h4>"
            if (tram.para.estado?.codigo == "E005") { //Archivado
                msg += "<p>El estado de su trámite es: <strong class='text-warning'><em>ARCHIVADO</em></strong></p>"
            } else {
                msg += "<p>El estado de su trámite es: <strong class='text-info'><em>${tram.estadoTramiteExterno?.descripcion ?: ''}</em></strong></p>"
            }
